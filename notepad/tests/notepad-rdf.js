...
test("when I query for its clusters", function() {
    // a service (an endpoint?) is a way to ask for something
    // var clusteringServiceEndpoint = "http://localhost:3030/dev/cluster";
    // clusteringServiceEndpoint.query(this.triples, function(results) {

    var clusters = $.notepad.cluster(this.triples);

    assertThat(clusters.length, greaterThan(0), "should receive at least one cluster");

    // Each subject should be in at least one cluster
    _.each(this.triples.subjects(), function(uri) {
        var belongingTriples = clusters.triples(undefined,"rdfs:member", uri);
        assertThat(belongingTriples.length, greaterThan(0), "should receive at least one member triple for that subject");
    });        
});